...
@router.post("/notes")
def add_patient_note(doctor_id: str, patient_id: str, note_text: str, current_user: dict = Depends(require_role("doctor", "admin")), db: Session = Depends(get_db)):
    if current_user["role"] == "doctor" and current_user["sub"] != doctor_id:
        raise HTTPException(403, "ليس لديك صلاحية")
...
def get_patient_notes(patient_id: str, doctor_id: str, current_user: dict = Depends(require_role("doctor", "admin")), db: Session = Depends(get_db)):
...
def get_availability(doctor_id: str, db: Session = Depends(get_db)):
    ...
    if not d:
        raise HTTPException(404, "الطبيب غير موجود")
    booked = db.query(Appointment).filter(Appointment.doctor_id == doctor_id, Appointment.status.in_(["confirmed", "pending"])).all()
    return {"available_days": d.available_days or [], "available_hours": d.available_hours or "", "booked_slots": [{"date": a.date, "time": a.time} for a in booked]}
...
def get_doctor_analytics(doctor_id: str, current_user: dict = Depends(require_role("doctor", "admin")), db: Session = Depends(get_db)):
    ...
    return {"favorites_count": fav_count, "total_appointments": total_apts, "weekly_bookings": weekly, "monthly_bookings": monthly, "unique_patients": patients, "monthly_appointments": [0]*12, "revenue_summary": {"this_month": revenue, "last_month": 0, "growth": 0}}
...
def add_prescription(doctor_id: str, patient_id: str, data: dict, current_user: dict = Depends(require_role("doctor", "admin")), db: Session = Depends(get_db)):
    ...
    pres = Prescription(id=f"pre_{uuid.uuid4().hex[:8]}", doctor_id=doctor_id, patient_id=patient_id, medications=data.get("medications", []), notes=data.get("notes", ""), created_at=now.isoformat())
    db.add(pres)
    db.add(MedicalRecord(id=f"rec_{uuid.uuid4().hex[:8]}", patient_id=patient_id, uploaded_by=doctor.name if doctor else "Doctor", type="prescription", title=f"وصفة طبية - د. {doctor.name if doctor else ''}", content=str(data.get("medications", [])), date=now.strftime("%Y-%m-%d"), created_at=now.isoformat()))
