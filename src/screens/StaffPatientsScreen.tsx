...
export default function StaffPatientsScreen({
    ...
    const loadData = async () => {
        ...
        try {
            const [apts, reqs] = await Promise.all([
                ...
                api.getDoctorHistoryRequests(doctorId),
            ]);
    ...
    const openPatientHistory = async (p: any) => {
        ...
        try {
            const [profile, nData, pData, vData, serviceReqs] = await Promise.all([
                ...
                api.getPatientNotes(doctorId!, p.id),
    ...
    const reloadPatientNotes = async (patientId: string) => {
        try {
            const [nData, vData, serviceReqs] = await Promise.all([
                api.getPatientNotes(doctorId!, patientId),
    ...
    const handleAddNote = async () => {
        ...
        try {
            await api.addPatientNote(doctorId!, selectedPatient.id, newNote);
            setNewNote('');
